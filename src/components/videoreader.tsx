...
function VideoReader({ link }: Video) {
    ...
    const [bg, setBg] = useState("url('src/assets/img/landing.jpg') center center /cover")

    const handleResize = () => {
        (window.innerWidth >= 768)?
        setBg("url('src/assets/img/video.jpeg') center center /cover"):
            setBg("url('src/assets/img/banner3.png') center center /cover");
    };
    ...
    return (
        <div className="flex flex-col relative  mt-14 w-full items-center">
            <div className="sm:max-w-md sm:h-80 md:max-w-2xl md:max-h-96 max-w-80 max-h-72 w-full rounded-3xl shadow-xl overflow-clip relative">
                ...
                <div className="absolute text-white sm:w-24 sm:h-24 w-20 h-20 flex flex-row justify-center items-center bg-blue-400 m-auto z-10 top-0 bottom-0 left-0 right-0 rounded-full shadow-lg">
